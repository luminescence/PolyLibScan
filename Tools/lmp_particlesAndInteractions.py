...
class particle_methods_bundled:
    def _find_particle_by_pdb_id(self, pdb_residue_id, molecule, consider_ghost_atoms=False):
        ...
        if len(particle)>1:
            ...
            raise Exception("There is no particle with chain %s, id %d, iCode %s and ghost status %s" % (pdb_residue_chain, pdb_residue_id[1], pdb_residue_id[2], (pdb_residue_id[0] != ' ')))
        return particle[0]
