...
class LmpController(object):
    ...
    def __init__(self, Id, parameters, paths, parametrisation, fifos, previous_instance=''):
        '''
        ...
        self.parametrisation['MD_parameters'].update(self.parameters['override_parameters'])
    ...
    def get_lmp_styles(self):
        styles = OrderedDict()
        globals_ = self.parametrisation['globals']
        ...
        styles['pair_style'] = self.add_pair_style(globals_, self.parametrisation['Pairs'])
        return styles
    ...
    def production_MD(self, temperature_production, time_bug_fix=True):
        ...
        if time_bug_fix:
            ...
            full_runs, timesteps_left = divmod(self.parametrisation['MD_parameters']['time_steps'], max_no_time_steps)

            for x in range(full_runs):
                self.instance.run(max_no_time_steps)
            self.instance.run(timesteps_left)
        else:
            self.instance.run(self.parametrisation['MD_parameters']['time_steps'])
    ...
    def model_specifications(self):
        # specify the model
        pair_coeffs = ['timestep 	%s' % self.parametrisation['MD_parameters']['timestep'],
                       'dielectric %s' % self.parametrisation['MD_parameters']['dielectric_par']]
        for pair in [pair for key,pair in self.parametrisation['Pairs'].items() if not pair['single_type_parametrised']]:
