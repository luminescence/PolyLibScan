...
class LmpController(object):
    ...
    def get_lmp_styles(self):
        ...
        styles['pair_style'] = self.add_pair_style(globals_, self.physical_model['Pairs'])
        return styles
    ...
    def model_specifications(self):
        ...
        for pair in [pair for key,pair in self.physical_model['Pairs'].items() if not pair['single_type_parametrised']]:
