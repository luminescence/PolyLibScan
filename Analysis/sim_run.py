...
class Run(plotting.Run):
    ...
    def write_full_trajectory(self, output_name):
        """Write full trajectory in xyz format to output_name.
        If it already exists, it will be removed before the trajectory is written."""
        try:
            os.remove(output_name)
        except OSError:  # in case file does not exist
            pass
        with open(output_name, 'a') as output_file:
            for ts, x in agnostic_tqdm(enumerate(self.trajectory())):
                output_file.write('%s \nGenerated by PolyLibScan \n' % (len(x)))
                snapshot_DF = pd.DataFrame(x['xyz'], index=x['type_'])
                snapshot_DF.to_csv(output_file, header=False, sep=' ')
